...
async function getHandler(req: NextRequest) {
  try {
    ...
    if (format === 'csv') {
      ...
      return new NextResponse(csv, {
        ...
        headers: {
          ...
          'Content-Disposition': `attachment; filename="dailycard-products.csv"`,
    ...
    console.error('DailyCard products viewer error:', error)
    return NextResponse.json(
      { success: false, message: 'Failed to load DailyCard products' },
      { status: 500 }
    )
  }
}
