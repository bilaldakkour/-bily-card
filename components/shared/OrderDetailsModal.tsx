...
export default function OrderDetailsModal({ order, open, onClose }: OrderDetailsModalProps) {
  ...
  return (
    <div
      className="fixed inset-0 z-[90] flex items-end justify-center bg-[radial-gradient(circle_at_top,rgba(14,165,233,0.10),transparent_28%),rgba(2,6,23,0.86)] p-3 backdrop-blur-md sm:items-center sm:p-4"
      role="dialog"
      aria-modal="true"
    ...
    >
      <div
        dir={isRTL ? 'rtl' : 'ltr'}
        className="relative w-full max-w-[42rem] overflow-hidden rounded-[30px] border border-cyan-300/12 bg-[linear-gradient(180deg,rgba(6,14,28,0.985),rgba(4,10,22,0.995))] shadow-[0_34px_90px_rgba(2,6,23,0.56)] ring-1 ring-white/[0.05]"
        onClick={(event) => event.stopPropagation()}
      >
        <div className={cn('pointer-events-none absolute inset-x-0 top-0 h-44 bg-gradient-to-b', statusMeta.glowClass)} />

        <div className="relative border-b border-white/8 px-4 py-3.5 sm:px-5">
          <div className="flex items-start justify-between gap-3">
            <div className="flex items-center gap-2">
              <button
                type="button"
                onClick={onClose}
                className="inline-flex h-10 w-10 items-center justify-center rounded-xl border border-white/10 bg-white/[0.04] text-slate-200 transition hover:border-rose-300/25 hover:bg-rose-400/10 hover:text-white"
              ...
              <button
                type="button"
                onClick={handleShare}
                className="inline-flex h-10 w-10 items-center justify-center rounded-xl border border-white/10 bg-white/[0.04] text-slate-200 transition hover:border-cyan-400/25 hover:bg-cyan-500/10 hover:text-cyan-100"
            ...
            <div className="min-w-0 text-right">
              ...
              <h2 id="order-details-modal-title" className="mt-1 text-xl font-black text-white sm:text-[1.55rem]">
                {copy.title}
              </h2>
        ...
        <div className="max-h-[84vh] overflow-y-auto px-3.5 py-3.5 sm:px-5 sm:py-5">
          <div className="rounded-[24px] border border-white/10 bg-[linear-gradient(180deg,rgba(255,255,255,0.04),rgba(255,255,255,0.02))] p-3.5 shadow-[inset_0_1px_0_rgba(255,255,255,0.03)]">
            <div className="flex items-start gap-3.5">
              <div className="flex h-20 w-20 shrink-0 items-center justify-center overflow-hidden rounded-[22px] border border-white/10 bg-white/[0.05] shadow-[0_18px_36px_rgba(2,6,23,0.26)]">
                {order.productImage ? (
              ...
              <div className="min-w-0 flex-1 text-right">
                <div className="flex flex-wrap items-center justify-end gap-2">
                  <span className={cn('inline-flex rounded-full border px-3 py-1 text-[11px] font-semibold', statusMeta.pillClass)}>
                    {statusLabel}
                  </span>
                  <span className="inline-flex rounded-full border border-white/10 bg-white/[0.03] px-3 py-1 text-[11px] font-semibold text-slate-300">
                    {copy.valueBand}: ${total}
                  </span>
                </div>

                <h3 className="mt-3 text-lg font-black text-white sm:text-xl">{order.productName}</h3>
                ...
                <div className="mt-3 grid grid-cols-2 gap-2 sm:flex sm:flex-wrap sm:justify-end">
                  ...
                  {order.selectedPackageOption ? (
                    <div className="col-span-2 rounded-2xl border border-cyan-400/14 bg-cyan-500/[0.08] px-3 py-2 text-right sm:col-span-1">
                      <p className="text-[10px] uppercase tracking-[0.16em] text-cyan-300">{copy.package}</p>
                      <p className="mt-1 text-sm font-semibold text-cyan-50">{order.selectedPackageOption}</p>
                    </div>
                  ) : null}
                </div>
              </div>
          ...
          <div className="mt-3.5 rounded-[24px] border border-white/10 bg-white/[0.03] px-3.5 py-4 sm:px-4">
            <div className="relative grid grid-cols-3 gap-2.5">
              <div
                className="pointer-events-none absolute left-[16.666%] right-[16.666%] top-[1.05rem] h-[2px] -translate-y-1/2 rounded-full bg-white/10"
                aria-hidden="true"
              >
                <span
                  className={cn(
              ...
              {stepLabels.map((label, index) => {
                ...
                return (
                  <div key={label} className="relative text-center">
                    <div
                      className={cn(
                        'relative mx-auto flex h-[36px] w-[36px] items-center justify-center rounded-full border backdrop-blur-sm',
                        circleClass
                      )}
                    >
                      {isDone ? (
                    ...
                    <p className={cn('mt-2 text-[11px] font-semibold sm:text-xs', isDone || isActive ? 'text-white' : 'text-slate-500')}>
                      {label}
                    </p>
                  </div>
                )
          ...
          <div className="mt-3.5 grid gap-3 sm:grid-cols-2">
            <div className="rounded-[20px] border border-white/10 bg-white/[0.03] px-3.5 py-3 text-right">
            ...
            <div className="rounded-[20px] border border-cyan-400/14 bg-[linear-gradient(135deg,rgba(34,211,238,0.08),rgba(37,99,235,0.10))] px-3.5 py-3 text-right">
              <div className="flex items-center justify-between gap-3">
                <div className="min-w-0 flex-1">
          ...
          <div className="mt-3.5 grid gap-3.5 lg:grid-cols-[minmax(0,1.08fr)_minmax(0,0.92fr)]">
            <div className="rounded-[22px] border border-white/10 bg-white/[0.03] p-3.5 text-right">
              ...
              <div className="rounded-[18px] border border-white/8 bg-slate-950/40 px-3.5 py-3">
                <p className="text-sm leading-7 text-slate-200">{noteText}</p>
              </div>

              {order.providerStatus ? (
                <div className="mt-2.5 flex items-center justify-between rounded-[18px] border border-white/8 bg-white/[0.03] px-3.5 py-3 text-sm">
          ...
          <div className={cn('mt-3.5 rounded-[22px] border px-4 py-3.5', statusMeta.panelClass)}>
            <div className="flex items-start justify-between gap-3">
              <StatusIcon className={cn('mt-0.5 h-[22px] w-[22px] shrink-0', statusMeta.iconClass)} />
              <div className="min-w-0 flex-1 text-right">
                ...
                <p className="mt-1 text-sm leading-6 text-current/85">{statusDescription}</p>
              </div>
            </div>
          </div>

          {shareNotice ? (
            <div className="mt-3 rounded-[18px] border border-cyan-400/18 bg-cyan-500/10 px-4 py-3 text-right text-sm text-cyan-100">
              {shareNotice}
            </div>
          ) : null}
        </div>
